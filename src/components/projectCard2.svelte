<script>
	export let title, description, tech, url, image, buttonText, logos = [];
	export let active = false;
    let urlHover = false
	let inactiveHovered = false
...
{#if !active}
	...
	on:mouseleave={() => {inactiveHovered = false}}>
		...
		<div class="absolute inset-0 bg-neutral-800 rounded-xl {inactiveHovered ? "opacity-30" : "opacity-[.6]"} transition-all" />
